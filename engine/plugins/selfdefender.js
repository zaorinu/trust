...
(function(){
            ...
            for (const m of muts) {
                if (m.type === 'attributes') {
                    ...
                    if (m.attributeName !== 'data-trust-protect' && m.attributeName !== 'integrity') {
                        console.warn('Trust self-defender: attribute changed', m);
                        KILL();
                    }
                } else if (m.type === 'childList') {
                    if (m.removedNodes.length || m.addedNodes.length) {
                        console.warn('Trust self-defender: DOM children changed', m);
                        KILL();
                    }
                }
            }
    ...
    console.log('%cTrust: self-defender plugin enabled','background:green;padding:2px 5px;border-radius:3px;font-weight:bold;');
})();
